        ...
        <div class="row">
            ...
            <div class="col-sm-12 col-xs-12">
                ...
                <div class="tab-content">
                    @foreach($languages as $language)
                        @php
                            ...
                            $productAttributeValueTranslation = $productAttributeValue->translations->where('locale', $language->code)->first() ?: null;
                        @endphp
                ...
                @if($productAttribute->type == 3)
                    ...
                    <div class="col-sm-6 col-xs-12">
                        @include('admin.partials.on_edit.form_fields.upload_file', ['model' => $productAttributeValue, 'deleteRoute' => route('admin.product-attribute.values.delete-image', ['id'=>$productAttribute->id, 'value_id' => $productAttributeValue->id])])
                    </div>
                @endif
                <div class="form form-horizontal">
                    <div class="form-body">
                        <hr>
                        @include('admin.partials.on_edit.position_in_site_button', ['model' => $productAttributeValue, 'models' => $productAttribute->values])
                    </div>
            ...
            @include('admin.partials.modals.positions_on_edit', ['parent' => $productAttribute->values, 'model' => $productAttributeValue])
        </div>
    </form>
@endsection
